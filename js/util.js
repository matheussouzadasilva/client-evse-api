...
class Util
{
    ...
    static logado()
    {
        ...
        if (xhr != undefined) {
            xhr.open("GET","http://localhost:9999/api/v1/logado", true);
            xhr.setRequestHeader("Content-Type", "application/json;charset=UTF-8");
            xhr.onreadystatechange = function() {
                 //Verificar pelo estado "4" de pronto.
                if (xhr.readyState == 4) {
                    if (xhr.status != 200) {
                        window.location = "http://localhost/client-evse-api/adm/formularios/form.login.htm"; 
    ...
    static logout()
    {
        ...
        if (xhr != undefined) {
            xhr.open("GET","http://localhost:9999/api/v1/logout", true);
            xhr.setRequestHeader("Content-Type", "application/json;charset=UTF-8");
            xhr.onreadystatechange = function() {
                 //Verificar pelo estado "4" de pronto.
                if (xhr.readyState == 4) {
                    Util.createCookie('token', '', -1);
                    window.location = "http://localhost/client-evse-api/site/paginas/home.htm";
                }
            }
